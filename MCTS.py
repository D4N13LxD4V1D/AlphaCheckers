...
class Node():
	...
	def __init__(self, state):
		...
		self.state = stateCpy
		self.playerTurn = stateCpy.playerTurn
		self.id = stateCpy.id
		self.edges = []
...
class MCTS():
	...
	def moveToLeaf(self):
		...
		while not currentNode.isLeaf():
			z += 1
			if z > 15:
				lg.logger_mcts.error("Current counter:")
				lg.logger_mcts.error(currentNode.state.counter)
				print("LOOPED MYSELF")

			lg.logger_mcts.info('PLAYER TURN...%d, %d', currentNode.state.playerTurn, currentNode.state.counter)
			...
			# tmp = copy.deepcopy(currentNode.state)
