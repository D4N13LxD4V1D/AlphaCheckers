...
class Game:
	...
	def identities(self, state, actionValues): 
		r"""
				Notes:
					flips gamestate to save all symetries
				Args:
...
class GameState:
	...
	def _binary(self):
		...
		Notes:
			...
			skupaj združi dve polji ki predstavljata lokacije figur
		Returns:
			??
	...
	def _getValue(self):
		...
		for x, y, z, a in self.winners:
			if self.board[x] + self.board[y] + self.board[z] + self.board[a] == 4 * -self.playerTurn:
				return -1, -1, 1 # middle value represents current player, last value represents opposite player
		return 0, 0, 0
	...
	def takeAction(self, action):
		...
		Returns:
			Tuple: newState-(Board representing new state), value-(1 previous player has won, 0 otherwise) , done(1 if game is finished, 0 otherwise)
